# ${{ values.app_name }} — Technical Documentation
...
| **Framework** | Flask |
| **Container registry** | Docker Hub (`jtandy/${{ values.app_name }}`) |
...
flowchart LR
    ...
    K8s -->|serve| App[${{ values.app_name }} Pod]
...
3. The CD job updates `charts/${{ values.app_name }}/values.yaml` with the new image tag and commits the change.
4. Argo CD syncs the `${{ values.app_name }}` application, rolling out the new image to the cluster.
...
├── charts/
│   ├── ${{ values.app_name }}/        # Helm chart for the application
│   │   ├── Chart.yaml
│   │   ├── values.yaml
...
```json
{
  ...
  "hostname": "${{ values.app_name }}-abc123",
...
```bash
docker build -f dockerfile -t ${{ values.app_name }}:local .
...
The primary deployment mechanism is the Helm chart at `charts/${{ values.app_name }}/`.

| Setting | Default | Description |
|---|---|---|
| `replicaCount` | `1` | Number of pod replicas |
| `image.repository` | `jtandy/${{ values.app_name }}` | Docker image repository |
...
| `ingress.hosts[0].host` | `${{ values.app_name }}.local` | Ingress hostname |
...
3. Builds and pushes `jtandy/${{ values.app_name }}:<short-sha>`
...
2. Updates `charts/${{ values.app_name }}/values.yaml` with the new image tag using `yq`
3. Commits the values change back to the repository
4. Installs the Argo CD CLI
5. Logs in to Argo CD and syncs the `${{ values.app_name }}` application
...
- **GitHub slug:** `jtandy13/${{ values.app_name }}`

Import this file into your Backstage catalog to surface the service in the developer portal.

## Operational Notes

- **Ingress host:** Add `${{ values.app_name }}-${{ values.env_name }}.local` to your hosts file or DNS to reach the service locally during development.
